...
def follow_line(frame):
    ...
    # IF LINE FOUND
    # ==========================================
    #print("c:"+current_color)
    #print("l:"+last_seen_color)
    #print(has_snapped)
    ...
    if is_entering and not has_snapped:
        ...
        # dy = 100  # height of ROI

        #angle = np.degrees(np.arctan2(diff, dy))
        # ==========================================
        # DECIDE TURN
        # ==========================================
        ...
        if abs(diff)>10:
            ...
            turn="soft"
        if diff > 0:
                entry_turn_direction = "RIGHT"
        else:
                entry_turn_direction = "LEFT"
        ...
        reset_pid_clock() # Good idea to reset the PID clock here too!
        return 0
                    
    elif is_exiting and has_snapped:
        has_snapped = False
        print(f"JUNCTION EXIT: Returning via {entry_turn_direction}")
        print(current_color)
        if entry_turn_direction is not None:
            print("Executing EXIT turn: " + turn)
            if turn=="soft":
                if entry_turn_direction == "RIGHT":
                    car.set_motors(40, -40)
                elif entry_turn_direction =="LEFT":
